********************************************************************************
* Table A8
* Robustness: candidates entry - excluding municipalities with two NLCE
* Output: $output/tables/TableA8.tex
********************************************************************************
...
local panel_A_prehead "\begin{table}[!h]\centering\begin{threeparttable}\scriptsize" ///
 "\caption{Concurrent national-local elections, candidates entry. Excluding municipalities that experience both concurrent elections}\label{tab:rob-exclude-two-nlse-n-cand}" ///
 "\begin{tabular}{lccccccccc} \hline\hline"
local panel_A_posthead "& \multicolumn{3}{c}{\textit{Number of candidates}} & \multicolumn{3}{c}{\textit{\shortstack{N. cand. of nationally-\\established party}}} & \multicolumn{3}{c}{\textit{\shortstack{N. cand. of \\independent party}}} \\ \cmidrule(lr){1-1} \cmidrule(lr){2-4} \cmidrule(lr){5-7} \cmidrule(lr){8-10}"
...
"Region by Year FE &&&\checkmark&&&\checkmark&&&\checkmark\\ \hline\hline " ///
"\end{tabular} \begin{tablenotes} \scriptsize{`notes'} \end{tablenotes} \end{threeparttable} \end{table}"
// first panel
esttab n_cand_1 n_cand_2 n_cand_3 n_natp_1 n_natp_2 n_natp_3 n_civp_1 n_civp_2 n_civp_3 using "$output/tables/TableA8.tex", ///
