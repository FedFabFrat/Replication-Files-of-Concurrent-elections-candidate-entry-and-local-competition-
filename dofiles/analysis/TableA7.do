********************************************************************************
* Table A7
* Robustness: electoral outcomes - Poisson estimator
* Output: $output/tables/TableA7.tex
********************************************************************************
...
* ele_out  *
qui ppmlhdfe sh_votes_civp_1st nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1, a(codcom ele_date i.codreg#i.anno) vce(cl codcom)
cap gen esam = 1 if e(sample) == 1
eststo clear
foreach y of varlist sh_votes_natp_1st sh_votes_civp_1st natwin civwin inv_hhi {
	qui eststo `y'_1: ppmlhdfe `y' nlse_did if ever_natp_1 == 1 & ever_civp_1 == 1 & esam == 1, a(codcom ele_date) vce(cl codcom)
	qui eststo `y'_2: ppmlhdfe `y' nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1 & esam == 1, a(codcom ele_date) vce(cl codcom)
	qui eststo `y'_3: ppmlhdfe `y' nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1 & esam == 1, a(codcom ele_date i.codreg#i.anno) vce(cl codcom)
...
local panel_A_prehead "\begin{table}[!h]\centering\begin{threeparttable}" ///
 "\caption{Concurrent national-local elections, electoral outcomes. Poisson estimator.}\label{tab:ele-out-pois}" ///
...
"Estimates employ a Poisson pseudo-likelihood estimator. Estimates include municipality and election date fixed effects, as well as region by year fixed effects. " + ///
...
// first panel
esttab sh_votes_natp_1st_1 sh_votes_natp_1st_2 sh_votes_natp_1st_3 sh_votes_civp_1st_1 sh_votes_civp_1st_2 sh_votes_civp_1st_3 using "$output/tables/TableA7.tex", ///
...
// second panel
esttab natwin_1 natwin_2 natwin_3 civwin_1 civwin_2 civwin_3 using "$output/tables/TableA7.tex", ///
...
// last panel
esttab inv_hhi_1 inv_hhi_2 inv_hhi_3 using "$output/tables/TableA7.tex", ///
tex compress nomtitles append nonumbers b (3) se starlevels(* 0.10 ** 0.05 *** 0.01) keep(nlse_did) coeflabels(nlse_did "\textit{NLCE\textsubscript{i,t}}") ///
