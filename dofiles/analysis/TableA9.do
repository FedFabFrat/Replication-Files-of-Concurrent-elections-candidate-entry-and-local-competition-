********************************************************************************
* Table A9
* Robustness: electoral outcomes - excluding municipalities with two NLCE
* Output: $output/tables/TableA9.tex
********************************************************************************
...
* identify municipalities that experience both concurrent elections *
bys codcom (ele_date): egen c = seq() if ever_natp_1 == 1 & ever_civp_1 == 1 & nlse == 1
bys codcom (ele_date): egen mac = max(c)
drop if mac == 2

* ele_out *
qui reghdfe sh_votes_civp_1st nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1, a(codcom ele_date i.codreg#i.anno) vce(cl codcom)
...
foreach y of varlist sh_votes_natp_1st sh_votes_civp_1st natwin civwin inv_hhi {
	...
	qui eststo `y'_2: reghdfe `y' nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1 & esam == 1, a(codcom ele_date) vce(cl codcom)
	qui eststo `y'_3: reghdfe `y' nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1 & esam == 1, a(codcom ele_date i.codreg#i.anno) vce(cl codcom)
...
local panel_A_prehead "\begin{table}[!h]\centering\begin{threeparttable}" ///
 "\caption{Concurrent national-local elections, electoral outcomes. Excluding municipalities that experience both concurrent elections}\label{tab:rob-exclude-two-nlse-ele-out}" ///
...
"The sample excludes those municipalities that experience the concurrency twice, i.e., both in 2001 and in 2008. " + ///
...
// first panel
esttab sh_votes_natp_1st_1 sh_votes_natp_1st_2 sh_votes_natp_1st_3 sh_votes_civp_1st_1 sh_votes_civp_1st_2 sh_votes_civp_1st_3 using "$output/tables/TableA9.tex", ///
...
// second panel
esttab natwin_1 natwin_2 natwin_3 civwin_1 civwin_2 civwin_3 using "$output/tables/TableA9.tex", ///
...
// last panel
esttab inv_hhi_1 inv_hhi_2 inv_hhi_3 using "$output/tables/TableA9.tex", ///
tex compress nomtitles append nonumbers b (3) se starlevels(* 0.10 ** 0.05 *** 0.01) keep(nlse_did) coeflabels(nlse_did "\textit{NLCE\textsubscript{i,t}}") ///
