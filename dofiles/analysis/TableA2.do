********************************************************************************
* Table A2
* Robustness: candidates entry with baseline controls interacted with year FEs
* Output: $output/tables/TableA2.tex
********************************************************************************
...
* n_cand - baseline contr *
qui reghdfe n_cand nlse_did if ever_natp_1 == 1 & ever_civp_1 == 1, a(codcom ele_date c.($controls_base)#i.anno i.codreg#i.anno) vce(cl codcom)
...
local panel_A_prehead "\begin{table}[!h]\centering\begin{threeparttable}\scriptsize" ///
 "\caption{Concurrent national-local elections, candidates entry. Baseline controls.}\label{tab:n-cand-basecontr}" ///
...
"Controls include: industry employment, agricultural employment, home-ownership rate, elderly dependency index, tertiary education rate, services employment, families below poverty line, unemployment rate. Controls are taken in 1991 and interacted with year fixed effects. " + ///
...
"Baseline controls $\times$ Year FE &&\checkmark&\checkmark&&\checkmark&\checkmark&&\checkmark&\checkmark\\ " ///
...
// first panel
esttab n_cand_1 n_cand_2 n_cand_3 n_natp_1 n_natp_2 n_natp_3 n_civp_1 n_civp_2 n_civp_3 using "$output/tables/TableA2.tex", ///
