********************************************************************************
* Figure 2
* Event studies: candidate entry (n_cand, n_civp, n_natp)
* Output: $output/figures/sa_n_cand.pdf, sa_n_civp.pdf, sa_n_natp.pdf
********************************************************************************
...
foreach y of varlist n_cand n_civp n_natp {
	if "`y'" == "n_cand" {
		local subfig "a"
		local yt "Number of candidates"
	}
	if "`y'" == "n_civp" {
		local subfig "b"
		local yt "N. of candidates of independent party"
	}
	if "`y'" == "n_natp" {
		local subfig "c"
		local yt "N. of candidates of nat.-established party"
	}
	...
	graph export "$output/figures/Figure2`subfig'.pdf", as(pdf) replace
}
