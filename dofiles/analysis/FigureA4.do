********************************************************************************
* Figure A4
* Robustness: relabelling - DiD table and event study
* Outputs: $output/figures/FigureA4.pdf
********************************************************************************
...
* relabelling event study *
qui reghdfe relabel nlse_did $controls if ever_natp_1 == 1 & ever_civp_1 == 1, a(codcom ele_date i.codreg#i.anno) vce(cl codcom)
...
recode nt .=0
eststo clear
eventstudyinteract relabel lag* lead* if ever_natp_1 == 1 & ever_civp_1 == 1 & esam == 1, vce(cluster codcom) absorb(codcom ele_date codreg#anno) cohort(fytb) control_cohort(nt) covariates($controls)
...
event_plot sa_b#sa_v, stub_lag(lag# lag#) stub_lead(lead# lead#) trimlead(3) trimlag(3) plottype(connected) ciplottype(rcap) together noautolegend ///
	...
	title("Probability of relabelling from independent to national") ///
	xlabel(-3(1)3) ///
...
graph export "$output/figures/FigureA4.pdf", as(pdf) replace
